...
const CONTRACT_ADDRESS =
  (import.meta.env.VITE_EXCHANGE_CONTRACT_ADDRESS as `0x${string}`) ||
  ("" as `0x${string}`);
...
export const CreateOrder = () => {
  ...
  const getOnchainBalance = async (): Promise<bigint> => {
    ...
    return (res as bigint) ?? 0n;
  };

  const depositIfNeeded = async (requiredAmount: bigint) => {
    if (!address) throw new Error("Wallet not connected");
    ...
    const missing = requiredAmount - balance;

    if (!writeContractAsync) {
      throw new Error(
        "writeContractAsync is not available. Make sure connector supports write actions."
      );
    }
    ...
    if (typeof txResult === "string") {
      ...
      if (!txHash && typeof txResult.wait === "function") {
        const waited = await txResult.wait?.(1);
        txHash = waited?.transactionHash ?? waited?.hash ?? txResult.hash;
      }
    }

    if (!txHash) {
      throw new Error(
        "Unable to obtain transaction hash after sending deposit"
      );
    }
  ...
  const handlePlaceOrder = async (side: "Buy" | "Sell") => {
    ...
    try {
      ...
      // For a BUY order, the required deposit is price * amount (both are in wei),
      // so multiply and scale down by 1e18 to keep result in wei.
      const requiredDeposit: bigint =
        side === "Buy" ? (priceWei * amountWei) / 1000000000000000000n : 0n;

      await depositIfNeeded(requiredDeposit);
      ...
      console.error(e);
      alert(e?.message || String(e) || "Transaction failed");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="bg-black/20 backdrop-blur-xl rounded-3xl border border-purple-dark/50 p-8 hover:border-purple-dark transition-all duration-300">
            <div className="space-y-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold italic tracking-wider">
                  Order Details
                </h3>
              ...
              {!isConnected && (
                <div className="mt-6 p-4 border-2 border-dashed border-purple-dark/30 rounded-xl text-center">
                  <p className="text-purple-light/40 italic mb-3">
          ...
          <div className="bg-black/20 backdrop-blur-xl rounded-3xl border border-purple-dark/50 p-8">
            <div className="space-y-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-purple-electric animate-pulse">⚡</div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="mt-8 p-6 rounded-xl bg-gradient-to-br from-purple-dark/20 to-crimson-dark/10 border border-purple-dark/30">
                  <h4 className="text-purple-electric font-bold mb-3">
                    Signing Process
                  </h4>
